    ...
    describe("Fee", function () {
        describe("ERC20 -> Native + ERC20 (Fee)", function () {
            it("TokenA(ERC20) -> Native + TokenB(ERC20) (Fee setup)", async function () {
                ...
                let feeRecepientNativeBalanceDiff = (await feeRecepient.getBalance()).sub(
                    feeRecepientNativeBalanceBefore,
                );
                expect(feeRecepientNativeBalanceDiff).to.equal(eth_amount_fee);

                let feeRecepientTokenSrcBalanceDiff = (await tercSrc.balanceOf(feeRecepient.address)).sub(
                    feeRecepientTokenSrcBalanceBefore,
                );
                expect(feeRecepientTokenSrcBalanceDiff).to.equal(token_amount_src_fee);

                let feeRecepientTokenDstBalanceDiff = (await tercDst.balanceOf(feeRecepient.address)).sub(
                    feeRecepientTokenDstBalanceBefore,
                );
