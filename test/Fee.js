    ...
    describe('Fee', function () {
        ...
        describe('ERC20 -> Native + ERC20 (Fee)', function () {
            it("TokenA(ERC20) -> Native + TokenB(ERC20) (Fee setup)", async function () {
                ...
                const { otmoic, owner, otherAccount, user, lp, feeRecepient } = await loadFixture(deployOtmoic);
                ...
                await otmoic.connect(owner).setTollAddress(feeRecepient.address);
                ...
                await expect(tercDst.connect(owner).transfer(lp.address, token_amount_dst))
                ...
                let feeRecepientNativeBalanceBefore = await feeRecepient.getBalance();
                ...
                let lpTokenDstBalanceBefore = await tercDst.balanceOf(lp.address);
                ...
                await expect(otmoic.connect(owner).confirmTransferOut(
                ...
                await expect(otmoic.connect(owner).confirmTransferIn(
                    lp.address,
                    user.address,
                    tercDst.address,
                    token_amount_dst,
                ...
                let feeRecepientNativeBalanceDiff = (await feeRecepient.getBalance()).sub(feeRecepientNativeBalanceBefore);
                expect(feeRecepientNativeBalanceDiff).to.equal(eth_amount_fee);

                let feeRecepientTokenSrcBalanceDiff = (await tercSrc.balanceOf(feeRecepient.address)).sub(feeRecepientTokenSrcBalanceBefore);
                expect(feeRecepientTokenSrcBalanceDiff).to.equal(token_amount_src_fee);

                let feeRecepientTokenDstBalanceDiff = (await tercDst.balanceOf(feeRecepient.address)).sub(feeRecepientTokenDstBalanceBefore);
                ...
                let userTokenDstBalanceDiff = (await tercDst.balanceOf(user.address)).sub(userTokenDstBalanceBefore);
                expect(userTokenDstBalanceDiff).to.equal(token_amount_dst.sub(token_amount_dst_fee));

                let lpNativeBalanceDiff = lpNativeBalanceBefore.sub(await lp.getBalance());
                // lpNativeBalanceDiff = eth_amount_fee + gas_fee_for_transferIn
                expect(lpNativeBalanceDiff).to.greaterThan(eth_amount_fee);

                let lpTokenSrcBalanceDiff = (await tercSrc.balanceOf(lp.address)).sub(lpTokenSrcBalanceBefore);
                expect(lpTokenSrcBalanceDiff).to.equal(token_amount_src.sub(token_amount_src_fee));

                let lpTokenDstBalanceDiff = lpTokenDstBalanceBefore.sub(await tercDst.balanceOf(lp.address));
